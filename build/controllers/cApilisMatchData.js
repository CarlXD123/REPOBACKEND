...
class ApilisMatchDataController {
    ...
    getAllMatchData(_req, _res) {
        try {
            ...
            let appointment = sApilisMatchData_1.default.getAllMatchData(start, limit);
            appointment.then((a) => {
                let result = {
                    "status": true,
                    "data": a
    ...
    getMatchData(_req, _res) {
        try {
            ...
            let category = sApilisMatchData_1.default.getMatchData(id);
            category.then((c) => {
                let result = {
                    "status": true,
                    "data": c
    ...
    addMatchData(_req, _res) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                let body = _req.body;
                yield sApilisMatchData_1.default.addMatchData(body);
                let result = {
                    "status": true,
                    "message": {
                        "code": "I034",
    ...
    updateMatchData(_req, _res) {
        try {
            let id = _req.params.id;
            let data = _req.body;
            const brand = sApilisMatchData_1.default.updateMatchData(id, data);
            brand.then((a) => {
                let result = {
                    "status": true,
                    "message": {
