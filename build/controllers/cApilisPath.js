...
class ApilisPathController {
    ...
    getAllPath(_req, _res) {
        try {
            ...
            const nation = sApilisPath_1.default.getAllPath(start, limit, query);
            nation.then((m) => {
    ...
    getPath(_req, _res) {
        try {
            ...
            let category = sApilisPath_1.default.getPath(id);
            category.then((c) => {
                let result = {
                    "status": true,
                    "data": c
    ...
    updatePath(_req, _res) {
        try {
            let id = _req.params.id;
            let data = _req.body;
            const brand = sApilisPath_1.default.updatePath(id, data);
            brand.then((a) => {
                let result = {
                    "status": true,
                    "message": {
