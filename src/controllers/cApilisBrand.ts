...
const mapperAgreement = new ApilisBrandCompleteMapper();
...
class ApilisBrandController {
    ...
    public getAllBrand(_req: Request, _res: Response) {
        try {
            ...
            const nation = sApilisBrand.getAllBrand(start, limit, query);
            nation.then((m: any) => {
                let result = {
                    "status": true,
                    ...m
    ...
    public getBrand(_req: Request, _res: Response) {
        try {
            ...
            brand.then(async (a: any) => {
                ...
                let mapeado: any = {};
                mapperAgreement.map<ApilisBrandAtributos, mApilisBrandComplete>(a, mapeado);
                if (bra != undefined)
                    mapeado.modelos = bra;
                result.data =mapeado;
                //console.log(result);
