...
class ApilisMatchDataController {
    ...
    public getAllMatchData(_req: Request, _res: Response) {
        try {
            ...
            let appointment = sApilisMatchData.getAllMatchData(start, limit);
    ...
    public getMatchData(_req: Request, _res: Response) {
        try {
            ...
            let category = sApilisMatchData.getMatchData(id);
            category.then((c: any) => {
                let result = {
                    "status": true,
                    "data": c
    ...
    public async addMatchData(_req: Request, _res: Response) {
        try {
            let body = _req.body;
            await sApilisMatchData.addMatchData(body);
            let result = {
                "status": true,
                "message": {
                    "code": "I034",
    ...
    public updateMatchData(_req: Request, _res: Response) {
        try {
            let id = _req.params.id;
            let data = _req.body;
            const brand = sApilisMatchData.updateMatchData(id, data);
            brand.then((a: any) => {
                let result = {
                    "status": true,
                    "message": {
